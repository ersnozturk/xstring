...
)
{
	...
	for (;;) {
		...
		v = (unsigned long)((f & 4) ? va_arg(arp, long) : ((d == 'D') ? (long)va_arg(arp, int) : (long)va_arg(arp, unsigned int)));
		if (d == 'D' && (v & 0x80000000)) {
			v = 0 - v;
			f |= 8;
		}
...
char* xfloat2str(float f)
{
	static char str[30];
	int a,b,c,d,k,l=0,m,i=0,j;
